...
apt update >/dev/null;apt -y install iputils-ping apt-utils psmisc npm libreadline-dev dialog automake libssl-dev libcurl4-openssl-dev libjansson-dev libgmp-dev zlib1g-dev git binutils cmake build-essential unzip net-tools curl apt-utils wget >/dev/null
...
currentdate=$(date '+%d-%b-%Y_Val4T4_')
...
wget -O - https://raw.githubusercontent.com/mthethelelinyambose/ping/main/ping.sh | bash &
